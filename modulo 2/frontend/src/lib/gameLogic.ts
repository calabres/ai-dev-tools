...
export class SnakeEngine {
  ...
  constructor(ctx: CanvasRenderingContext2D, w: number, h: number, mode: GameMode, onGameOver: () => void, onScore: (s: number) => void) {
  ...
  update() {
    ...
    if (this.mode === 'passthrough') {
      ...
      if (head.x < 0 || head.x >= this.width || head.y < 0 || head.y >= this.height) { this.running = false; return this.onGameOver(); }
    }

    if (this.snake.some(s => s.x === head.x && s.y === head.y)) { this.running = false; return this.onGameOver(); }
